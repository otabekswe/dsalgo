...
Keling endi `__init__`dagi `self.head`ga node qo'shish uchun method yaratamiz va uni nomini `append` deb qo'yamiz.
...
print(my_list.head.value) # 1
print(my_list.head.next.value) # 2
print(my_list.head.next.next) # None
```
...
Keling boshqa methodlarini ham kiritib chiqsak va ularni keyin tahlil qilsak:
